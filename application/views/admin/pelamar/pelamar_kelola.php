...
$id_posisi 	= empty($loker) ? null : $loker->id_posisi;
...
<div class="box box-default box-solid">
	<div class="box-header with-border">
		...
		<?=form_open('admin/pelamar/add_pelamar', array('role'=>'form', 'class'=>'inline'))?>
			...
			<button type="submit" name="edit_pelamar" class="btn btn-info pull-right" data-placement='top' title='Edit status'>
				<i class="glyphicon glyphicon-plus"></i> Tambah Pelamar
			</button>
		<?=form_close()?>
    </div>
	...
	<!--<div class="box-header with-border">
  	...
  	<div class="box-body">
		...
		<div class="form-group">
            <label for="inputEmail3" class="col-sm-5 control-label">Posisi</label>									
			<div class="col-sm-5">
                <label class="control-label"><?php echo ": ".$posisi; ?></label>
            </div>
        </div>
		...
		<div class="form-group">
			...
			<div class="col-sm-5">
                <label class="control-label"><?php echo ": ".tgl_indo($tgl_buka); ?></label>
            </div>
        </div>
		...
		<div class="form-group">
			...
			<div class="col-sm-5">
                <label class="control-label"><?php echo ": ".tgl_indo($tgl_tutup); ?></label>
            </div>
        </div>				
	</div>
</div>
                        ...
                        <?php $i=1; foreach ($pelamar as $row) :?>
                            <tr>
								...
								<?php switch($row->status_pelamar){
									...
									case 4: echo "<font color='#00c0ef'>Lolos administrasi (diundang tes)</font>";break;
							...
							-->	
								...
								<?=form_open('admin/pelamar/edit_status_lolos', array('role'=>'form', 'class'=>'inline'))?>
									<input type="hidden" name="id_pelamar" value="<?=$row->id_pelamar?>">
									<input type="hidden" name="id_loker" value="<?=$id_loker?>">
									<input type="hidden" name="nik" value="<?=$row->nik?>">
									<button type="submit" name="edit_pelamar" class="btn btn-sm" data-placement='top' title='Edit status lolos administrasi'>
										<i class="glyphicon glyphicon-ok-circle" style='color: orange;'></i>
									</button>
								<?=form_close()?>

								<?=form_open('admin/pelamar/edit_status_diterima', array('role'=>'form', 'class'=>'inline'))?>
									<input type="hidden" name="id_pelamar" value="<?=$row->id_pelamar?>">
									<input type="hidden" name="id_loker" value="<?=$id_loker?>">
									<input type="hidden" name="nik" value="<?=$row->nik?>">
									<button type="submit" name="edit_pelamar" class="btn btn-sm" data-placement='top' title='Edit status diterima'>
										<i class="glyphicon glyphicon-saved" style='color: green;'></i>
									</button>
								<?=form_close()?>
								...
								<?=form_open('admin/pelamar/delete_pelamar', array('role'=>'form', 'class'=>'inline'))?>
									<input type="hidden" name="id_pelamar" value="<?=$row->id_pelamar?>">
									<input type="hidden" name="id_loker" value="<?=$id_loker?>">
									<button type="submit" class="btn btn-sm" title='Hapus pelamar' onclick="return confirm('Anda Yakin ?');">
