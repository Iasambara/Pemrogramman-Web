...
class laporan extends ci_controller{
	...
	function export_industri(){
		$data['title'] = 'Data Mitra Industri';
		$data['industri'] = $this->db->query("select i.*,p.*,k.*,b.* from t_industri i inner join t_provinsi p on i.id_prov=p.id_prov inner join t_kabupaten k on i.id_kabkot=k.id_kabkot inner join t_bidang b on i.id_bidang=b.id_bidang order by i.id_industri")->result();
		$this->load->view('admin/laporan/export_industri',$data);		
//		$data['file']='admin/laporan/cetak_industri';
//		$this->load->view('admin/main',$data);		
	}
	...
	function laporan_pelamar(){
		if(!empty($this->input->post('id_loker'))){
			...
			if($id_loker=='1'){
				$sql="select p.nik,s.jk,s.tahun_keluar,i.nama as nama_industri,s.nama as nama_siswa,l.judul,p.status from t_pelamar p inner join t_siswa s on p.nik=s.nik inner join t_loker l on l.id_loker=p.id_loker inner join t_industri i on i.id_industri=l.id_industri";
    			$data['title'] = 'Laporan Pelamar';
			}else{
				$sql="select p.nik,s.jk,s.tahun_keluar,i.nama as nama_industri,s.nama as nama_siswa,l.judul,p.status from t_pelamar p inner join t_siswa s on p.nik=s.nik inner join t_loker l on l.id_loker=p.id_loker inner join t_industri i on i.id_industri=l.id_industri where p.id_loker='$id_loker'";
                $namaind = $this->db->query("select l.judul,i.nama from t_loker l, t_industri i where l.id_industri=i.id_industri and l.id_loker=$id_loker")->row();
        		$data['title'] = 'Laporan Data Pelamar '.$namaind->nama;
			}
			...
			$data['lap']=$this->db->query($sql)->result();
			// update temp_table
			$this->db->query("drop table IF EXISTS tmp_lap_pelamar");
			$this->db->query("create table tmp_lap_pelamar AS ".$sql);
	...
	function export_pelamar($nmi=''){
	    if ($nmi == "")
    		$data['title'] = 'Laporan Data Pelamar';
		else {
            $namaind = $this->db->query("select l.judul,i.nama from t_loker l, t_industri i where l.id_industri=i.id_industri and l.id_loker=$nmi")->row();
    		$data['title'] = 'Laporan Data Pelamar '.$namaind->nama;
		}
		$data['pelamar'] = $this->db->query("select * from tmp_lap_pelamar")->result();
		//echo "LOLOS";
		$this->load->view('admin/laporan/export_pelamar',$data);		
	}
	...
	function laporan_pelamar_diterima(){
		if(!empty($this->input->post('id_loker'))){
			$id_loker=$this->input->post('id_loker');
			if($id_loker=='1'){
				$sql="select p.nik,s.jk,s.tahun_keluar,i.nama as nama_industri,s.nama as nama_siswa,l.judul,p.status from t_pelamar p inner join t_siswa s on p.nik=s.nik inner join t_loker l on l.id_loker=p.id_loker inner join t_industri i on i.id_industri=l.id_industri where p.status='2'";
			}else{
				$sql="select p.nik,s.jk,s.tahun_keluar,i.nama as nama_industri,s.nama as nama_siswa,l.judul,p.status from t_pelamar p inner join t_siswa s on p.nik=s.nik inner join t_loker l on l.id_loker=p.id_loker inner join t_industri i on i.id_industri=l.id_industri where p.status='2' and p.id_loker='$id_loker'";
			}
